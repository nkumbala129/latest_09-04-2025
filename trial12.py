...
if not st.session_state.authenticated:
    ...
    def is_structured_query(query: str):
        structured_patterns = [
            r'\b(select|from|where|group by|order by|join|sum|count|avg|max|min|least|highest|which|what)\b',
            r'\b(total|revenue|sales|profit|projects|county|jurisdiction|month|year|energy savings|kwh)\b'
        ]
